...
class Grid extends Control
{
	...
	/** @var bool */
	public $enablePaginator = FALSE;

	/** @var bool */
	public $enableSorting = FALSE;

	/** @var bool */
	public $enableCounting = FALSE;
	 ...
	 * @throws DuplicateButtonException
	 */
	protected function addEditButton($label = NULL)
	{
		if (!empty($this["buttons"]->components["edit"])) {
			throw new DuplicateButtonException("Button $name already exists.");
		}

		$button = new Button($this["buttons"], "edit");
		$self = $this;
		$primaryKey = $this->primaryKey;
		$button->setLink(function($row) use ($self, $primaryKey) {
			return $self->link("this", array("id" => $row[$primaryKey]));
		});

		$button->setLabel($label);
		$button->setClass("edit");
	...
	{
		if (!empty($this["buttons"]->components["delete"])) {
			throw new DuplicateButtonException("Button $name already exists.");
		}

		$button = new Button($this["buttons"], "delete");
		$self = $this;
		$primaryKey = $this->primaryKey;
		$button->setLink(function($row) use ($self, $primaryKey) {
			return $self->link("delete", array("id" => $row[$primaryKey]));
		});
	...
	public function render()
	{
		...
		$this->template->paginate = $this->enablePaginator;
		$this->template->enableCounting = $this->enableCounting;
	...
	{		
		$args = func_get_args();
 		return $this->parent->link($args[0], (isset($args[1]) ? $args[1] : NULL));
	}
	...
	public function handleDelete($id)
	{
		...
		$this->flashMessage("Záznam byl smazán.","flash-success");
